...
const ENERGY_CENTERS = [
  { id: "EC001", name: "SolarHub North", city: "Delhi", lat: 28.7, lng: 77.1, stored: 4200, capacity: 6000, producers: 14 },
  { id: "EC002", name: "WindCore East",  city: "Kolkata", lat: 22.5, lng: 88.3, stored: 2800, capacity: 5000, producers: 9 },
  { id: "EC003", name: "BioGreen South", city: "Chennai", lat: 13.0, lng: 80.2, stored: 3600, capacity: 4500, producers: 11 },
  { id: "EC004", name: "HydroBase West", city: "Mumbai",  lat: 19.0, lng: 72.8, stored: 5100, capacity: 7000, producers: 18 },
];

const PRODUCERS_INIT = [
  { id: "P001", name: "Ravi Solar Farm", type: "Solar", city: "Delhi", pricePerUnit: 4.2, unitsAvailable: 800, earnings: 12400, centerId: "EC001" },
  { id: "P002", name: "Green Wind Co.",  type: "Wind",  city: "Kolkata", pricePerUnit: 3.8, unitsAvailable: 1200, earnings: 9800,  centerId: "EC002" },
  { id: "P003", name: "BioMass Ltd.",    type: "Biogas",city: "Chennai", pricePerUnit: 5.1, unitsAvailable: 500,  earnings: 7200,  centerId: "EC003" },
  { id: "P004", name: "SunRise Rooftop", type: "Solar", city: "Mumbai", pricePerUnit: 4.5, unitsAvailable: 320,  earnings: 6100,  centerId: "EC004" },
];

const CONSUMERS_INIT = [
  { id: "C001", name: "Amit Sharma",   type: "Household", city: "Delhi",   monthlyUsage: 240, centerId: "EC001", connected: true,  monthlyBill: 1008 },
  { id: "C002", name: "TechCorp India",type: "Industry",  city: "Kolkata", monthlyUsage: 8400, centerId: "EC002", connected: true, monthlyBill: 31920 },
  { id: "C003", name: "Priya Reddy",   type: "Household", city: "Chennai", monthlyUsage: 180, centerId: "EC003", connected: true,  monthlyBill: 918 },
];
...
function calcConnectionCost(type, city) {
  const nearest = ENERGY_CENTERS.reduce((a, b) => {
...
// ─── CONSUMER PORTAL ─────────────────────────────────────────────────────────
function ConsumerPortal({ consumers, setConsumers, producers }) {
  ...
  const getQuote = () => {
    ...
    const q = calcConnectionCost(form.connType, form.city);
    setQuote(q);
  };
  ...
  const buyConnection = () => {
    ...
    const newC = {
      id, name: form.name, type: form.connType, city: form.city,
      monthlyUsage: 0, centerId: quote.centerId, connected: true,
      monthlyBill: 0, address: form.address, pricePerUnit,
      connectionCost: quote.cost,
    };
    setConsumers(prev => [...prev, newC]);
    setMyId(id);
    setStep("myAccount");
  };
      ...
      <div style={S.section}>
        ...
        <div style={{ ...S.grid4, marginBottom: 24 }}>
          {[
            ...
            { label: "Energy Center", value: ENERGY_CENTERS.find(e => e.id === myConsumer.centerId)?.name, icon: "grid" },
        ...
        <div style={S.grid2}>
          <div style={S.cardGlow}>
            ...
            <UsageSimulator consumer={myConsumer} setConsumers={setConsumers} />
          </div>
          <div style={S.card}>
            ...
            <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#78909c" }}>{c.city} · {c.type} · Center: {ENERGY_CENTERS.find(e => e.id === c.centerId)?.name}</div>
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
...
// ─── USAGE SIMULATOR ─────────────────────────────────────────────────────────
function UsageSimulator({ consumer, setConsumers }) {
  const [units, setUnits] = useState("");
  const bill = units ? (parseFloat(units) * (consumer.pricePerUnit || 4.5)).toFixed(2) : null;

  const submitUsage = () => {
    if (!units) return;
    setConsumers(prev => prev.map(c => c.id === consumer.id
      ? { ...c, monthlyUsage: parseFloat(units), monthlyBill: parseFloat(bill) }
      : c));
    setUnits("");
...
// ─── PRODUCER PORTAL ─────────────────────────────────────────────────────────
function ProducerPortal({ producers, setProducers, consumers }) {
  ...
  const registerProducer = () => {
    ...
    const nearest = ENERGY_CENTERS.reduce((a, b) => getDistance(form.city, a.city) < getDistance(form.city, b.city) ? a : b);
    if (editId) {
      setProducers(prev => prev.map(p => p.id === editId ? { ...p, ...form, pricePerUnit: parseFloat(form.pricePerUnit), unitsAvailable: parseInt(form.unitsAvailable), centerId: nearest.id } : p));
    } else {
      setProducers(prev => [...prev, {
        id: "P" + Date.now().toString().slice(-4), ...form,
        pricePerUnit: parseFloat(form.pricePerUnit),
        unitsAvailable: parseInt(form.unitsAvailable),
        earnings: 0, centerId: nearest.id,
      }]);
    }
    setStep("market");
    setEditId(null);
    ...
    <div style={S.section}>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {producers.map(p => {
            const center = ENERGY_CENTERS.find(e => e.id === p.centerId);
...
// ─── ADMIN PORTAL ─────────────────────────────────────────────────────────────
function AdminPortal({ producers, consumers, centers, setCenters }) {
  ...
  const doTransfer = () => {
    ...
    const fromCenter = centers.find(c => c.id === transferForm.from);
    if (fromCenter.stored < amt) { alert("⚠️ Insufficient stored energy!"); return; }
    setCenters(prev => prev.map(c => {
      if (c.id === transferForm.from) return { ...c, stored: c.stored - amt };
      if (c.id === transferForm.to) return { ...c, stored: Math.min(c.capacity, c.stored + amt) };
      return c;
    }));
    const log = { from: centers.find(c => c.id === transferForm.from)?.name, to: centers.find(c => c.id === transferForm.to)?.name, amount: amt, time: new Date().toLocaleTimeString() };
    setTransferLog(prev => [log, ...prev].slice(0, 5));
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {transferLog.map((t, i) => (
                <div key={i} style={{ padding: "10px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 8, fontSize: 12 }}>
                  ...
                  <div style={{ color: "#90a4ae" }}>{t.from} → {t.to}</div>
                  <div style={{ color: "#78909c", fontSize: 11 }}>at {t.time}</div>
                </div>
              ))}
            </div>
          )}
...
export default function App() {
  ...
  const [consumers, setConsumers] = useState(CONSUMERS_INIT);
  const [producers, setProducers] = useState(PRODUCERS_INIT);
  const [centers, setCenters] = useState(ENERGY_CENTERS);

  // Sync producers into centers dynamically
  useEffect(() => {
    setCenters(prev => prev.map(c => ({
      ...c,
  ...
  return (
    <div style={S.app}>
      ...
      <main>
        {portal === "consumer" && <ConsumerPortal consumers={consumers} setConsumers={setConsumers} producers={producers} />}
        {portal === "producer" && <ProducerPortal producers={producers} setProducers={setProducers} consumers={consumers} />}
        {portal === "admin" && <AdminPortal producers={producers} consumers={consumers} centers={centers} setCenters={setCenters} />}
      </main>
    </div>
  );
}
